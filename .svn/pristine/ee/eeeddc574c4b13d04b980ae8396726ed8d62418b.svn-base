...
import com.gbicc.company.warnDisposal.operation.TCmWarnTaskStartProOperation;
...
public class TCmWarnTaskStartProUpdate extends BaseUpdate {
...
//			UpdateResultBean updateResultBean3 = multiUpdateResultBean.getUpdateResultBeanByID(DATASET_ID3);
			...
			if (updateResultBean.hasNext()) {
				...
				if(map.get("opinion")!=null){
					oc.setAttribute(TCmWarnTaskStartProOperation.OPINION, map.get("opinion"));
				}
				if(StringUtils.isBlank(id)) {
					dd = new TCmWarnTask();
					oc.setAttribute(TCmWarnTaskStartProOperation.CMD,TCmWarnTaskStartProOperation.CMD_INSERT);
				} else {
					dd = TCmWarnTaskService.getInstance().get((String) map.get("id"));
					oc.setAttribute(TCmWarnTaskStartProOperation.CMD,TCmWarnTaskStartProOperation.CMD_UPDATE);
				}
				mapToObject(dd, map);
			}
			oc.setAttribute(TCmWarnTaskStartProOperation.IN_PARAM, dd);
...
//			}
			...
			OPCaller.call(TCmWarnTaskStartProOperation.ID, oc);
