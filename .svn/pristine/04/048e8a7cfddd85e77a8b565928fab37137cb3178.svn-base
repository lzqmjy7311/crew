...
import com.gbicc.company.view.externaldata.entity.TDataGsxtIcPunishment;
...
public class TDataGsxtIcPunishmentGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		StringBuffer querySql=new StringBuffer("SELECT  L.ID id,L.DECIDE_WRIT fdPunishmentDocNum, L.BREAK_TYPE fdPunishmentType, L.PUNISH_CONTENT fdPunishmentContent, L.DECIDE_ORG fdPunishmentOrg, L.ROW_DATE fdUpdateTime, L.BASE_ID fdJbkId,V.REGISTOR_NO fdRegNum,V.COMPANY_NAME fdName, V.SHARE_HD_TYPE fdType, V.LEGAL_NAME fdLecalPerson, V.REGISTOR_AMT fdRegCapital, V.CHECK_DATE fdSetUpDate, V.ADDRESS fdEntAddress, V.BUSSTART_DATE fdBusinessTermStart, V.BUSEND_DATE fdBusinessTermEnd, V.BUSSCOPE fdBusinessScope, V.ENTER_ORG fdRegOrg, V.CHECK_DATE fdApprovalDate, V.ENTER_STATUS fdRegStatus,  V.SHARE_HD_TYPE fdShareholderType, V.SHARE_HD_NAME fdShareholder, V.CERT_TYPE fdIdType, V.CERT_NO fdIdNum,v.RELA_ID   FROM ECUSER.T_OUT_IND_PUNISH_DETAIL L LEFT JOIN T_WB_GSINFO_V V ON L.BASE_ID=V.RELA_ID WHERE 1=1");
...
//				hql.toString());
		...
		List<TDataGsxtIcPunishment> listT=jt.query(sqlp.toString(), BeanPropertyRowMapper.newInstance(TDataGsxtIcPunishment.class));
