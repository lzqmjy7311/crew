...
public class UserMgrService {
	 ...
	 * @return 校验结果信息，如果用户不存在或用户存在但密码不对，抛出异常；校验通过，返回true
	 */
	public TlrInfo checkUser(String userLoginId, String password, String userBrno) throws CommonException {
		if (logger.isDebugEnabled()) {
...
//				List list = hqldao.queryByQL2List("from TlrBctlRel where brNo='" + userBrno + "' and tlrNo='"+userLoginId+"'");
//				if (list.size() <= 0) {
//					ExceptionUtil.throwCommonException("选择登录的机构未授权");
//				}

				List<Bctl> bctls = hqldao.queryByQL2List("from Bctl where brno='" + userBrno + "'");
				if(bctls.size() <= 0){
...
//					ExceptionUtil.throwCommonException("操作员还未到生效期", ErrorCode.ERROR_CODE_TLR_NO_EFFECT);
//				}
//				if (globalInfo.getTxdate().after(tlrInfo.getExpireDate())) {
//					ExceptionUtil.throwCommonException("操作员已无效", ErrorCode.ERROR_CODE_TLR_IS_EXPIRE);
//				}
...
//				if ((SystemConstant.ERR_PWD_TIMES_SUM <= pswdErrCnt) && (true == txDate.equals(pwdErrDate))) {
	...
	private void checkPassword(TlrInfo user, String password,
		...
		if (PswdValidteOP.LOCKED.equals(user.getIsLock())) {
			...
			if (lockingTime < 0) {
				...
				long x = System.currentTimeMillis() - DateUtil.StringToTime(user.getLastfailedtm()).getTime();
		...
		if (!passwdService.ComparePassword(password, user.getPassword(), enc)) {
			...
			if (PswdValidteOP.LOCKED.equals(user.getIsLock())) {
				...
				ExceptionUtil.throwCommonException("用户名或密码错误!", ErrorCode.ERROR_CODE_USER_PWD_INVALID);
...
//					globalInfo.setTlrStatus(SystemConstant.TLR_NO_STATE_LOGIN);
					tlrInfo.setLastaccesstm(DateUtil.Time14ToString2(DateUtil.getTimestamp()));
...
//					globalInfo.setRoleId(tlrInfo.getRoleid().intValue());
	 ...
	 * @return
	 */
	public boolean setLoginOutInfo(GlobalInfo globalInfo) throws CommonException {
		if (logger.isDebugEnabled()) {
		...
		try {
			// 修改数据库查询方式
			TlrInfoDAO tlrInfoDAO = BaseDAOUtils.getTlrInfoDAO();
			tlrInfo = tlrInfoDAO.queryById(globalInfo.getTlrno());
			if (null != tlrInfo) {
				// 设置为登出状态
//				globalInfo.setTlrStatus(SystemConstant.TLR_NO_STATE_LOGOUT);
				...
				// 最近登出时间
				tlrInfo.setLastlogouttm(DateUtil.Time14ToString2(DateUtil.getTimestamp()));
//				tlrInfo.setRoleid(tlrInfo.getRoleid());
	...
	public ArrayList getUserRoles(String tlrNo) throws CommonException {
		...
		try {
			...
			sb.append("select role from ").append("TlrRoleRel tr,RoleInfo role ").append("where tr.roleId=role.id ").append(
					"and tr.tlrno='").append(tlrNo).append("' ");
			Iterator iterator;
	...
	public List getUserOrgs(final String tlrno) throws CommonException{
		List<Bctl> list=ROOTDAOUtils.getROOTDAO()
				.queryListHQL("from Bctl where brcode in ( select tlrBctl.brNo from TlrBctlRel tlrBctl "
						+ " where tlrBctl.tlrNo = '" + tlrno + "' )");
		return list;
	}
	...
	public ArrayList getUserFunctions(String tlrNo) throws CommonException {
		...
		try {
			...
			HQLDAO hqlDAO = BaseDAOUtils.getHQLDAO();
			StringBuffer sb = new StringBuffer();
			sb.append("select distinct func from ").append("TlrRoleRel tr,RoleFuncRel rr,FunctionInfo func,TlrInfo lf ").append(
					"where  tr.roleId=rr.roleId and rr.funcid=func.id ")
					.append(" and lf.tlrno=tr.tlrno and lf.roleid=tr.roleId ")
					.append(" and tr.tlrno='").append(tlrNo)
					.append("' and func.funcType=0")
	 ...
	 * @return
	 */
	public ArrayList getUserFunctions(String tlrNo, String pid) throws CommonException {
		if (logger.isDebugEnabled()) {
			logger.debug("getUserFunctions(String) - start"); //$NON-NLS-1$
		}
		try { 
			...
			sb.append("select distinct func from ").append("TlrRoleRel tr,RoleFuncRel rr,FunctionInfo func,TlrInfo lf ").append(
					...
					.append(StringUtils.isBlank(pid) ? "and func.lastdirectory = '0' " : "and func.lastdirectory ='" + pid + "'").append(" and func.funcType=0").append(" order by func.showseq");
			Iterator iterator = hqlDAO.queryByQL(sb.toString());
			while (iterator.hasNext()) {
				FunctionInfo func = (FunctionInfo) iterator.next();
				if(func.getIconCls()==null){
	 ...
	 * @return
	 */
	public ArrayList getAllUserFunctions(String tlrNo) throws CommonException {
		if (logger.isDebugEnabled()) {
			logger.debug("getUserFunctions(String) - start"); //$NON-NLS-1$
		}
		try {
	    	...
	    	hql.append("select distinct func from ").append("TlrRoleRel tr,RoleFuncRel rr,FunctionInfo func,TlrInfo lf ").append(
					"where  tr.roleId=rr.roleId and rr.funcid=func.id ")
					.append(" and lf.tlrno=tr.tlrno and lf.roleid=tr.roleId ")
					.append(" and tr.tlrno='").append(tlrNo)
					.append("'  order by func.showseq");
			Iterator iterator = hqlDAO.queryByQL(hql.toString());
			while (iterator.hasNext()) {
				FunctionInfo func = (FunctionInfo) iterator.next();
				if(func.getIconCls()==null){
					func.setIconCls("");
	...
	public UserSessionInfo getUserSessionInfo(TlrInfo tlrinfo) throws CommonException {
		...
		try {
			...
			sessionInfo.setLastLogoutTime(tlrinfo.getLastlogouttm());
			...
			if(lastSucList != null && lastSucList.size() > 0){
				sessionInfo.setLastLoginTime(lastSucList.get(0).getLoginSucTm());
			} else {
				sessionInfo.setLastLoginTime(tlrinfo.getLastaccesstm());
			}
			//设置最近登录失败时间
			...
			if (lastFailList != null && lastFailList.size() > 0) {
				sessionInfo.setLastLoginFailTime(lastFailList.get(0).getLoginFailTm());
			}
		} catch (CommonException e) {
			throw e;
		} catch (Exception e) {
	 ...
	 * @param userBrno
	 */
	public UserSessionInfo loginUserSessionInfo(String userId, String pwd, String userBrno) throws CommonException {
		// 检查用户密码
		TlrInfo tlrinfo = checkUser(userId, pwd, userBrno);
		return getUserSessionInfo(tlrinfo);
	}

	public UserSessionInfo loginUserSessionInfo(String userId,String userBrno) throws CommonException {
		if (logger.isDebugEnabled()) {
			logger.debug("checkUser(String, String) - start"); //$NON-NLS-1$
		}
		GlobalInfo globalInfo = GlobalInfo.getCurrentInstance();
		try {
			TlrInfoDAO tlrInfoDAO = BaseDAOUtils.getTlrInfoDAO();
			List tlrList = tlrInfoDAO.queryByCondition("tlrno = '" + userId + "'");
			if (tlrList.isEmpty()) {
				ExceptionUtil.throwCommonException(ErrorCode.ERROR_CODE_USER_NOT_EXIST);
			} else if (tlrList.size() != 1) {
				logger.error("TlrInfo[" + userId + "] existed mutil");
				ExceptionUtil.throwCommonException(ErrorCode.ERROR_CODE_USER_PWD_INVALID);
			} else {
				TlrInfo tlrInfo = (TlrInfo) tlrList.get(0);
				tlrInfoDAO.getHibernateTemplate().evict(tlrInfo);
	 ...
	 */
//	public void changeRole(Integer curRoleId, UserSessionInfo userSessionInfo) throws CommonException {
...
//		RoleInfo roleInfo = roleInfoService.getRoleInfoByRoleId(curRoleId.intValue());
//		String workfoleRole = "";//roleInfo.getWorkflowRole();删除role_info中的workflow_role字段
//		// 设置操作员当前岗位
...
//		sb.append("select fi.funcCode").append(" from RoleFuncRelation as rfr, FunctionInfo as fi ").append("where rfr.roleId=").append(
...
//	}
	...
	public List<RoleInfo> getUserRoleByOrgId(String userId,String orgId){
		...
		sql.append("select distinct t1.F_ROLE_ID as ROLE_ID from T_USER_ORGROLE_REL t1 "+
			" where t1.F_USER_ID='"+userId+"' "+
			" and t1.F_ORG_ID='"+orgId+"' ");
		try {
			List<Integer> list=(List<Integer>) com.gbicc.common.CommonService
					.getInstance().findSqlSimpleListBySpringJdbc(sql.toString(), Integer.class);
			StringBuffer ins = new StringBuffer();
			for(Integer in:list){
				ins.append(in).append(",");
			}
			String in=ins.substring(0,ins.lastIndexOf(","));
			return DAOUtils.getRoleInfoDAO().queryByCondition(" id in ("+in+")");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<Map<String,Object>> getUserAllOrg(String userId){
		StringBuffer sql = new StringBuffer();
		sql.append("select t1.F_ORG_ID as BR_NO,t1.F_ROLE_ID as ROLE_ID from T_USER_ORGROLE_REL t1"+
				" where t1.F_USER_ID='"+userId+"' ");
		try {
			return com.gbicc.common.CommonService
	...
	public List<TlrInfo> getUserAllUser(String orgId,String qtlrnoName,String qtlrno){
		StringBuffer sql = new StringBuffer();
		StringBuffer wh=new StringBuffer("");
		if(StringUtils.isNotBlank(qtlrno)){
			wh.append(" and o.TLRNO='"+qtlrno+"' ");
		}
		if(StringUtils.isNotBlank(qtlrnoName)){
			wh.append(" and o.TLR_NAME like '%"+qtlrnoName+"%' ");
		}
		sql.append("select * from TLR_INFO O where 1=1"+wh+" and o.TLRNO in(SELECT T1.F_USER_ID  from T_USER_ORGROLE_REL t1"+
				" where t1.F_ORG_ID='"+orgId+"' and F_USER_ID not in (select o.TLRNO from TLR_INFO o where o.BRCODE='"+orgId+"'))");
		try {
			return com.gbicc.common.CommonService
					.getInstance().getJdbcTempldate().query(sql.toString(), BeanPropertyRowMapper.newInstance(TlrInfo.class));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return new ArrayList<TlrInfo>();
	}
	 ...
	 * @return
	 */
	public String getRellUser(String loanNo){
		String sql="select bankId from T_EDW_PLS_ACCOUNT_V where LOANACNO='"+loanNo+"' ";
		return (String) com.gbicc.common.CommonService
				.getInstance().findSqlOneBySpringJdbc(sql, String.class);
	}
}
