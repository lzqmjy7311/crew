...
public class OperMngEntryGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		String hql = "1=1 ";
		if (!"00001".equals(brcode)) {//upt 总行不过滤 
		...
		List<TlrInfo> list1=UserMgrService.getInstance().getUserAllUser(brcode,qtlrnoName,qtlrno);
		
		Iterator<TlrInfo> it1=list1.iterator();
		while(it1.hasNext()){
			TlrInfo tlrInfotemp=new TlrInfo();
			tlrInfoList.add(it1.next());
		}
		/** 对最后一页的处理 */
