...
<@CommonQueryMacro.LayoutPanel id="rule" inbody="false" >
	...
	function btQuery_onClick(){	
		...
		ZtMmfxy_dataset.setParameter("coreCustomerNum",testid);
...
//	}
	...
	function DynamicTree_tree123_onSelect(e,node){
		...
		if(node.pid!=null&&node.pid!=''){
			...
			acctId=node.id;
			//alert(node.pid);
		}else if((node.pid==null||node.pid=='')&&node.id!=null){
//			if((DynamicTree_tree123.getRoot() !=null)&&(DynamicTree_tree123.getRoot() !=undefined)){
				account='';
				acctId='';
				acctType='';
				testid=node.id;
