...
public class TPlYtReportOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		TPlYtReport dd = context.getAttribute(IN_PARAM) == null ? null : (TPlYtReport) context.getAttribute(IN_PARAM);
...
//					report.setMainSurvPer(tlrInfo.getTlrName());//主调查人
//				}
				//20160823更改，检查责任人查不到时不再读取默认设置用户，改为当前经办人
				tlrInfo=TlrInfoService.getInstance().getTlrInfoByTlrno(info.getOperid());
				if(tlrInfo!=null){
					plYtMonitor.setHandler(tlrInfo);
					report.setMainSurvPer(tlrInfo.getTlrName());//主调查人
				}else{
...
//					.queryListHQL("from TPlYtReport where id in (select reportId from TPlReportRelation where businessId='"+businessId+"') ").get(0);
			List<TPlYtReport> reportList = CommonService.getInstance()
					.queryListHQL("from TPlYtReport where id in (select reportId from TPlReportRelation where businessId='"+businessId+"') ");
			if(reportList == null || reportList.size()<=0){
				return;
			}
			TPlYtReport report = reportList.get(0);
