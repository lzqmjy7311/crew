...
public class TCwCreditAssetsMonitorGetter extends BaseGetter {
	...
	public PageQueryResult getData() throws Exception {
		...
		sbf.append(" 	) T INNER JOIN T_CW_IDNEX_MANAGE TI ON T.INDEX_CODE=TI.INDEX_CODE and T.INDEX_TYPE='T4' AND T.GATHER_TYPE=TI.GATHER_TYPE ");
		...
		sbf.append("SELECT T1.ORG_CODE,T1.INDEX_CODE,T1.INDEX_NAME,T1.INDEX_TYPE,T1.MONITOR_FRE,T1.INDUSTRY_CODE,T1.NOW_VAL,T1.COLO,T1.THRESHOLD1,T1.THRESHOLD2,T2.NOW_VAL TOP_VAL,T3.NOW_VAL YC_VAL ,T1.DATA_DATE "
				...
				+" WHERE T1.DATA_DATE=TO_CHAR(DATE('"+DateUtils.getNowDateLastQ(new Date())+"')-1 DAYS,'YYYY-MM-DD') AND T1.GATHER_TYPE='3' AND T1.INDEX_TYPE='T2' ) X ");
