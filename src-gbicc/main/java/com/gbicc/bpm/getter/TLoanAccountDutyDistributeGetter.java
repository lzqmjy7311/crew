...
public class TLoanAccountDutyDistributeGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		StringBuffer hql = new StringBuffer("from TLoanAccountDutyDistribute t where 1=1 and distType='"+TLoanAccountDutyDistributeOperation.DIST_TYPE_DUTY+"' ");
		...
		hql.append(" and t.operOrg.brno in ("+orgs+")");
		...
		for(int i=0;i<list.size();i++){
			...
			String[] userGroup=distribute.getCheckUser().split(",");
			String user="";
			...
			TlrInfo tlr=TlrInfoService.getInstance().getTlrInfoByTlrno(user);
