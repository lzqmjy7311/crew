 ...
 * @desc   按揭类定期监控报告查询
 */
public class TPlDqReportAjByIdGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		StringBuffer hql = new StringBuffer("from TPlDqReportAj t where 1=1 and id=(select reportId from TPlReportRelation where businessId='"+businessId+"') ");
		...
		if(StringUtils.isNotEmpty(businessId)){
			...
			if(!list.isEmpty()){
				TPlDqReportAj t=(TPlDqReportAj) list.get(0);
				//20160825更改，不再从后台读取卡号，因为一个客户可能对应多个卡号，直接从前台获取卡号
				t.setLoanId(loanAcct);
			}
		}
