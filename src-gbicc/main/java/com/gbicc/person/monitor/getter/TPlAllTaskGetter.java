...
public class TPlAllTaskGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		String userId=info.getTlrno().trim();
		boolean headerOrg=info.isHeadBrcode();
		String roleId=info.getWorkflowRoleId().trim();
		TPlTaskService tPlTaskService=TPlTaskService.getInstance();
		int pageSize = getResult().getPage().getEveryPage();
		int pageIndex = getResult().getPage().getCurrentPage();
		String brcode= info.getBrcode().trim();//TODO 机构
		...
		StringBuffer sql=tPlTaskService.getQueryAllStr(userId, roleId,null);
		...
		if(taskType.equals(TaskType.TODOTASK)||
				...
				taskType.equals(TaskType.OVERTASK)||
				taskType.equals(TaskType.BACKTASK)){
			sqlp.append(" and currentUser ='").append(userId).append("'");
		}
		String where="";
		if(!"7".equals(rptStatus)){
			where=tPlTaskService.getQueryStr(taskType);
		}
		sqlp.append(where);
		//找出用户 当前机构
		String orgs=BctlService.getInstance().findParentOrgIDsTree(brcode);
		//////////TODO 根据用户当前机构过滤//////////////
		if(!headerOrg){
			if(orgs.length()>0){
