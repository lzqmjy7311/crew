...
public class OldCheckUserGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		if(StringUtils.isNotEmpty(flag) && flag.equals("all")){
			resultList=getListByCondition(condition,bankid,type);
		}else{
			if(StringUtils.isNotEmpty(type) && type.equals(TLoanAccountDutyDistributeOperation.DIST_TYPE_COLL)){
				resultList=getCollListByLoanacnos(loanacnos);
			}else{
	 ...
	 * @return
	 */
	public List<Map<String,Object>> getListByCondition(String condition,String bankid,String type){
		String sql="";
		if(StringUtils.isNotEmpty(type) && type.equals(TLoanAccountDutyDistributeOperation.DIST_TYPE_COLL)){
			sql="SELECT DISTINCT CASE WHEN D.FD_COLLID IS NOT NULL THEN B1.BRCODE "+ 
			"WHEN A.DUTYID2 IS NOT NULL THEN TLR.BRCODE ELSE '' END AS BRCODE, "+
			"CASE WHEN D.FD_COLLID IS NOT NULL THEN B1.BRNAME WHEN A.DUTYID2 IS NOT NULL THEN B2.BRNAME ELSE '' END AS BRNAME,"+
			...
			"ON A.DUTYID2=TLR.TLRNO LEFT JOIN TLR_INFO TLR2 ON D.FD_COLLID=TLR2.TLRNO LEFT JOIN BCTL B1 ON TLR2.BRCODE=B1.BRCODE "+ 
			...
			"AND LOANACNO NOT IN (SELECT FD_LOANACNO FROM T_PL_ACCOUNT_DUTY_DISTRIBUTE WHERE FD_DIST_TYPE='"+type+"' AND FD_STATUS='"+TLoanAccountDutyDistributeOperation.STATUS_APL_ING+"' AND FD_DIST_FLAG='"+TLoanAccountDutyDistributeOperation.DIST_FLAG_SINGLE+"') "+
			"AND LOANACNO NOT IN (SELECT T2.FD_LOANACNO FROM T_PL_ACCOUNT_DUTY_DISTRIBUTE T1 "+
			"JOIN T_PL_ACCOUNT_REL_DISTRIBUTE T2 ON T1.FD_ID=T2.FD_DIST_ID WHERE FD_DIST_TYPE='"+type+"' AND T1.FD_DIST_FLAG='"+TLoanAccountDutyDistributeOperation.DIST_FLAG_MUCH+"' AND FD_STATUS='"+TLoanAccountDutyDistributeOperation.STATUS_APL_ING+"')";
		}else{
			sql="SELECT DISTINCT CASE WHEN D.FD_DUTYID IS NOT NULL THEN B1.BRCODE "+ 
	 ...
	 * @return
	 */
	public List<Map<String,Object>> getCollListByLoanacnos(String loanacnos){
		List<Map<String,Object>> resultList=null;
		if(StringUtils.isNotEmpty(loanacnos)){
			String[] loannos=loanacnos.split(",");
			String loanno="";
			...
			String sql="SELECT DISTINCT"+
			"  CASE "+
			"  		WHEN D.FD_COLLID IS NOT NULL THEN B1.BRCODE "+
			"  		WHEN A.DUTYID2 IS NOT NULL THEN TLR.BRCODE "+
			"  		ELSE ''  "+
			"  END AS BRCODE, "+
			"  CASE "+
			"  		WHEN D.FD_COLLID IS NOT NULL THEN B1.BRNAME "+
			"  		WHEN A.DUTYID2 IS NOT NULL THEN B2.BRNAME "+
			"  		ELSE '' "+
			"  END AS BRNAME, "+
			"  CASE "+
			"  		WHEN D.FD_COLLID IS NOT NULL THEN D.FD_COLLID "+
			"  		WHEN A.DUTYID2 IS NOT NULL THEN A.DUTYID2 "+
			"  		ELSE '' "+
			"  END AS TLRNO, "+
			"  CASE "+
			"  		WHEN D.FD_COLLID IS NOT NULL THEN D.FD_COLLNAME "+
			"  		WHEN A.DUTYID2 IS NOT NULL THEN TLR.TLR_NAME "+
			"  		ELSE '' "+
			"  END AS TLRNAME "+
			"FROM T_EDW_PLS_ACCOUNT A  "+
			"LEFT JOIN T_PL_ACCOUNT_REL_DUTY D "+
			"ON A.LOANACNO=D.FD_LOANACNO "+
			"LEFT JOIN TLR_INFO TLR "+
			"ON A.DUTYID2=TLR.TLRNO "+
			"LEFT JOIN TLR_INFO TLR2 "+
			"ON D.FD_COLLID=TLR2.TLRNO "+
			"LEFT JOIN BCTL B1 "+
			"ON TLR2.BRCODE=B1.BRCODE "+
			"LEFT JOIN BCTL B2 "+
			"ON TLR.BRCODE=B2.BRCODE "+
