			...
			if(dutyDistribute.getDistFlag().equals(TLoanAccountDutyDistributeOperation.DIST_FLAG_SINGLE)){//单帐户分派
				...
				if(checkUserQuantity>1){
					if(dutyDistribute.getDistPrinciple().equals(DIST_PRINCIPLE_1)){
						...
						while(items.hasNext()){
							for(int i=0;i<recordList.size();i++){
								if(items.hasNext()){
									...
									saveRelDuty(relDuty,recordList.get(i).getBrcode().getBrno()+","+recordList.get(i).getTlrno().getTlrno(),type,relDist.getLoanacno(),dutyDistribute.getOldCheckUser());
									items.remove();
								}
							}
						}
						...
						for(int i=0;i<recordList.size();i++){
							String hql="select count(loanacno) from TLoanAccountRelDuty where dutyid='"+recordList.get(i).getBrcode().getBrno()+","+recordList.get(i).getTlrno().getTlrno()+"' ";
							Integer count=rootdao.queryByHqlToCount(hql);
							map.put(recordList.get(i).getBrcode().getBrno()+","+recordList.get(i).getTlrno().getTlrno(),count);
					...
					for(int i=0;i<relDistList.size();i++){
						TLoanAccountRelDistribute relDist= (TLoanAccountRelDistribute) relDistList.get(i);
						TLoanAccountRelDuty relDuty=relDutyService.get(relDist.getLoanacno());
						saveRelDuty(relDuty,recordList.get(0).getBrcode().getBrno()+","+recordList.get(0).getTlrno().getTlrno(),type,relDist.getLoanacno(),dutyDistribute.getOldCheckUser());
					}
				}
			}
		}
