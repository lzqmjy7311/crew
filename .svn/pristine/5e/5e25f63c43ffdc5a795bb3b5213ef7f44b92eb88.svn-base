...
<@CommonQueryMacro.page title="信贷资产监测指标展示_机构排名">
<style>
div.form1{
	width:90%;
	height:250px;
...
function datatable1_opr_onRefresh(cell,value,record) {
	if (record) {//当存在记录时
		cell.innerHTML="<a href='JavaScript:selectIndexReport(\""+record.getValue("INDEX_CODE")+"\",\""+record.getValue("ORG_CODE")+"\",\""+record.getValue("ORG_NAME")+"\",\""+record.getValue("GATHER_TYPE")+"\")'>查看</a>";
	} else {//当不存在记录时
		cell.innerHTML="&nbsp;";
	}
}
...
function selectIndexReport(codeVal,ORG_CODE,ORG_NAME,GATHER_TYPE){
	parent.parent.GTab.addTab("selectCustomerWin"+GATHER_TYPE+ORG_CODE,"信贷资产监测指标展示_机构_"+ORG_NAME,"/gbicc-report-pages/combinationWarn/ftl/TCwIndexOrgReport.ftl?INDEX_CODE="+codeVal+"&GATHER_TYPE="+GATHER_TYPE+"&ORG_CODE="+ORG_CODE);
}
