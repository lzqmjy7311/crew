...
//		
		...
		if (CMD_QUERY.equals(cmd)) {
			...
			List<TEdwPlsCust> list2=CommonService.getInstance()
				.findSqlListBySpringJdbc("select * from T_EDW_PLS_CUST  where CUSTID='"+yjReport.getFdCustCode()+"'", TEdwPlsCust.class);
			TEdwPlsCust cust=list2.get(0);
			yjReport.setCustPhone(cust.getMobilecall());
			yjReport.setCustAddress(cust.getContaddr());
...
//					tPlCreditMemo.setStartRemark(startRemark);//发起原因
		...
		map.put("desc",BpmDescUtil.getDesc(businessId,jyReport.getFdLoanAccount(),jyReport.getFdCustName()));
