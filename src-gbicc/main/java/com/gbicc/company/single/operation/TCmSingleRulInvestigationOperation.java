...
public class TCmSingleRulInvestigationOperation extends BaseOperation {
	...
	public void execute(OperationContext context) throws CommonException {
		...
		try {
			if(op.equals(OP_SAVE_INVE)){
				if(StringUtils.isEmpty(investigation.getInveCode())){
					String sql="SELECT 'XC'||to_char(current date,'YYYYMMDD')||substr('000'||SINGLE_RUL_INVE_NO.nextval,length('000'||SINGLE_RUL_INVE_NO.nextval)-2) FROM dual";
					Iterator it=rootdao.queryBySQL(sql);
					while(it.hasNext()){
						investigation.setInveCode(it.next().toString());
					}
				}else if(investigation.getInveNumb()>1){
					String sql="SELECT N.FD_SAVEFLAG FROM T_CM_SINGLE_RUL_INVESTIGATION N WHERE N.FD_ID='"+investigation.getId()+"' ";
					String saveFlag="";
					Iterator it=rootdao.queryBySQL(sql);
					while(it.hasNext()){
						saveFlag=it.next().toString();
					}
					if(!saveFlag.equals("1")&&!investigation.getSaveFlag().equals("2")){
						//重调查的提交，保存本次协查内容，2016-03-08提出的需求：分派的协查如第三次分派重新调查的任务，建议重新调查的界面要显示前1次重新调查的内容（灰色，不可更改）。
						String delSql="delete T_CM_SINGLE_RUL_INVE_HIST where FD_INVE_ID='"+investigation.getId()+"' ";
						rootdao.executeSql(delSql);
						String insertSql="insert into T_CM_SINGLE_RUL_INVE_HIST select uuid(),fd_id,FD_CHECK_EMPH,FD_INVE_WAY,FD_PART_USER,FD_INVE_FEED_DT,FD_INVE_CONC from t_cm_single_rul_investigation where FD_ID='"+investigation.getId()+"' ";
						rootdao.executeSql(insertSql);
					}
				}
				...
				if(investigation.getInveNumb()>1){
					//20160919修改，如果已经保存过内容，则不再执行保存动作
					if(!investigation.getSaveFlag().equals("1")){
						//重调查的提交，保存本次协查内容，2016-03-08提出的需求：分派的协查如第三次分派重新调查的任务，建议重新调查的界面要显示前1次重新调查的内容（灰色，不可更改）。
						String delSql="delete T_CM_SINGLE_RUL_INVE_HIST where FD_INVE_ID='"+investigation.getId()+"' ";
						rootdao.executeSql(delSql);
						String insertSql="insert into T_CM_SINGLE_RUL_INVE_HIST select uuid(),fd_id,FD_CHECK_EMPH,FD_INVE_WAY,FD_PART_USER,FD_INVE_FEED_DT,FD_INVE_CONC from t_cm_single_rul_investigation where FD_ID='"+investigation.getId()+"' ";
	...
	public void startWarningInveProcess(TCmSingleRulInvestigation investigation,TCmSingleRulCases cases) throws Exception{
		...
		assigneeList.add(investigation.getInveUser().getTlrno());
