package com.gbicc.company.single.outputentity;
// Generated 2016-1-15 16:51:03 by Hibernate Tools 3.2.2.GA


import java.math.BigDecimal;

/**
 * TRewL01IndBaseInfoId generated by hbm2java
 */
public class L01  implements java.io.Serializable {

	private static final long serialVersionUID = -5503881512969606894L;
	
	/*
    * ID.
    */
    private String id;
    /*
    * 企业名称.
    */
    private String companyName;
    /*
    * 注册号.
    */
    private String registorNo;
    /*
    * 法定代表人.
    */
    private String legalName;
    /*
    * 证件号码.
    */
    private String certNo;
    /*
    * 注销日期/吊销日期.
    */
    private String cancelDate;
    /*
    * 登记状态.
    */
    private String enterStatus;
    /*
    * 经营期限起始日.
    */
    private String busstartDate;
    /*
    * 经营期限结束日.
    */
    private String busendDate;
    /*
    * 经营范围.
    */
    private String busscope;
    /*
    * 注册资本.
    */
    private String registorAmt;
    /*
    * 登记机关.
    */
    private String enterOrg;
    /*
    * 住所.
    */
    private String address;
    /*
    * 核准日期.
    */
    private String checkDate;
    /*
    * 股东.
    */
    private String shareHolder;
    /*
    * 原链接.
    */
    private String sourceLink;
    /*
    * 数据日期.
    */
    private String rowDate;
    /*
    * 关联ID.
    */
    private String relaId;
    /*
    * 预警表ID.
    */
    private String warnId;
    /*
    * 数据日期.
    */
    private String dataDt;

    public L01() {
    }

	/**
    * Get the ID
    */	
    public String getId() {
        return this.id;
    }
	/**
    * Set the ID
    */	
    public void setId(String id) {
        this.id = id;
    }
	/**
    * Get the 企业名称
    */	
    public String getCompanyName() {
        return this.companyName;
    }
	/**
    * Set the 企业名称
    */	
    public void setCompanyName(String companyName) {
        this.companyName = companyName;
    }
	/**
    * Get the 注册号
    */	
    public String getRegistorNo() {
        return this.registorNo;
    }
	/**
    * Set the 注册号
    */	
    public void setRegistorNo(String registorNo) {
        this.registorNo = registorNo;
    }
	/**
    * Get the 法定代表人
    */	
    public String getLegalName() {
        return this.legalName;
    }
	/**
    * Set the 法定代表人
    */	
    public void setLegalName(String legalName) {
        this.legalName = legalName;
    }
	/**
    * Get the 证件号码
    */	
    public String getCertNo() {
        return this.certNo;
    }
	/**
    * Set the 证件号码
    */	
    public void setCertNo(String certNo) {
        this.certNo = certNo;
    }
	/**
    * Get the 注销日期/吊销日期
    */	
    public String getCancelDate() {
        return this.cancelDate;
    }
	/**
    * Set the 注销日期/吊销日期
    */	
    public void setCancelDate(String cancelDate) {
        this.cancelDate = cancelDate;
    }
	/**
    * Get the 登记状态
    */	
    public String getEnterStatus() {
        return this.enterStatus;
    }
	/**
    * Set the 登记状态
    */	
    public void setEnterStatus(String enterStatus) {
        this.enterStatus = enterStatus;
    }
	/**
    * Get the 经营期限起始日
    */	
    public String getBusstartDate() {
        return this.busstartDate;
    }
	/**
    * Set the 经营期限起始日
    */	
    public void setBusstartDate(String busstartDate) {
        this.busstartDate = busstartDate;
    }
	/**
    * Get the 经营期限结束日
    */	
    public String getBusendDate() {
        return this.busendDate;
    }
	/**
    * Set the 经营期限结束日
    */	
    public void setBusendDate(String busendDate) {
        this.busendDate = busendDate;
    }
	/**
    * Get the 经营范围
    */	
    public String getBusscope() {
        return this.busscope;
    }
	/**
    * Set the 经营范围
    */	
    public void setBusscope(String busscope) {
        this.busscope = busscope;
    }
	/**
    * Get the 注册资本
    */	
    public String getRegistorAmt() {
        return this.registorAmt;
    }
	/**
    * Set the 注册资本
    */	
    public void setRegistorAmt(String registorAmt) {
        this.registorAmt = registorAmt;
    }
	/**
    * Get the 登记机关
    */	
    public String getEnterOrg() {
        return this.enterOrg;
    }
	/**
    * Set the 登记机关
    */	
    public void setEnterOrg(String enterOrg) {
        this.enterOrg = enterOrg;
    }
	/**
    * Get the 住所
    */	
    public String getAddress() {
        return this.address;
    }
	/**
    * Set the 住所
    */	
    public void setAddress(String address) {
        this.address = address;
    }
	/**
    * Get the 核准日期
    */	
    public String getCheckDate() {
        return this.checkDate;
    }
	/**
    * Set the 核准日期
    */	
    public void setCheckDate(String checkDate) {
        this.checkDate = checkDate;
    }
	/**
    * Get the 股东
    */	
    public String getShareHolder() {
        return this.shareHolder;
    }
	/**
    * Set the 股东
    */	
    public void setShareHolder(String shareHolder) {
        this.shareHolder = shareHolder;
    }
	/**
    * Get the 原链接
    */	
    public String getSourceLink() {
        return this.sourceLink;
    }
	/**
    * Set the 原链接
    */	
    public void setSourceLink(String sourceLink) {
        this.sourceLink = sourceLink;
    }
	/**
    * Get the 数据日期
    */	
    public String getRowDate() {
        return this.rowDate;
    }
	/**
    * Set the 数据日期
    */	
    public void setRowDate(String rowDate) {
        this.rowDate = rowDate;
    }
	/**
    * Get the 关联ID
    */	
    public String getRelaId() {
        return this.relaId;
    }
	/**
    * Set the 关联ID
    */	
    public void setRelaId(String relaId) {
        this.relaId = relaId;
    }
	/**
    * Get the 预警表ID
    */	
    public String getWarnId() {
        return this.warnId;
    }
	/**
    * Set the 预警表ID
    */	
    public void setWarnId(String warnId) {
        this.warnId = warnId;
    }
	/**
    * Get the 数据日期
    */	
    public String getDataDt() {
        return this.dataDt;
    }
	/**
    * Set the 数据日期
    */	
    public void setDataDt(String dataDt) {
        this.dataDt = dataDt;
    }
}