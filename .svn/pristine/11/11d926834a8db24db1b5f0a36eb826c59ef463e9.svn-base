...
<@CommonQueryMacro.LayoutPanel id="cc" >
    <@CommonQueryMacro.LayoutCenter title="操作员列表">
		<@CommonQueryMacro.CommonQuery id="operMngEntry" init="false" submitMode="current">
			<@CommonQueryMacro.DataTable id ="datatable1" toolbar="mytoolbar" paginationbar="-,btModify,-,btResetPwd2,-,btLoginStatus,-,unLock,-,btExport" fieldStr="tlrno[80],tlrName[100],flag[80],status[80],isLock[75],brname[180],roles[200],lastaccesstm[100],lastlogouttm[100],st[80]"  readonly="true" width="100%" height="100%" />
...
<script language="javascript">
	...
	function btModifyShow(tlrno){
		//debugger;
	  	...
	  	var foo = operMngEntry_dataset.getValue("tlrno");
		if(foo!=""){
			currentSubWin = openSubWin("pageWinId1", "", "/fpages/regonization/ftl/OperMngRoleInfo1.ftl?op=modify&tlrno="+foo+"&brcode="+brcode,"950","640");
		}else{
			alert("请先选中要修改的记录！");
		}
	}
