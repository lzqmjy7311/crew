...
30	1	1	结束标识	#
 */
public class ClientChannelInterceptor extends _Entry<IChannelInterceptor>
...
//		byte[] allBodyMsg = ArrayUtils.addAll(xmlhead.getBytes(), outMsg);
		ctx.setOutMsgByte(ArrayUtils.addAll(xmlhead.getBytes(), outMsg));
		try{
			ctx.setOutMsg(new String(ctx.getOutMsgByte(),ctx.getEncoding()));
		}catch(Exception e){
			throw new AppException(Module.SYSTEM_MODULE, Rescode.DEFAULT_ERROR, "ClientChannelInterceptor doPostRequest Fail",e);
		}
	}
