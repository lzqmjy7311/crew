...
public class TPlTaskService {
	...
	public StringBuffer getQueryAllStr(String userId,String roleId,String brcode){
		...
		if(StringUtils.hasText(roleId)){
			...
			}else if(roleId.equals("557")||roleId.equals("558")||roleId.equals("111")||roleId.equals("615")||roleId.equals("614"))
			{
				...
				String brcodes = null;
				try {
					brcodes = bctlService.getAllSubListStr(brcode);
				} catch (CommonException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				TPlTaskService tts=TPlTaskService.getInstance();
				userIds=tts.getUserIdsByBrcodes(brcodes)	;
			}
		}
		StringBuffer sql=new StringBuffer("select L1.BRNAME,t3.*,row_number() over(PARTITION by t1.PROC_INST_ID_ order by t2.START_TIME_ desc) rk, t2.START_TIME_ as CREATE_TIME_,t2.ASSIGNEE_ currentUser, t6.TLR_NAME currentUserName,case when t2.ASSIGNEE_ in ("+userIds+") then '1' else  '0' end pageReadOnly, t4.TLR_NAME HANDLER_NAME,t3.HANDLER HANDLER_NO,t5.BRCODE HANDLERORGNO,t5.BRNAME HANDLERORGNAME ");
        ...
        sql.append(" JOIN  TLR_INFO t4 on t3.HANDLER=t4.TLRNO ");
        sql.append(" JOIN  BCTL t5 on t5.BRCODE=t4.BRCODE ");
        sql.append(" JOIN  TLR_INFO t6 on t6.TLRNO=t2.ASSIGNEE_ ");
        sql.append(" LEFT JOIN  BCTL L1 on L1.BRCODE=T3.BANK_ID ");
        if(userId!=null){
	        sql.append(" where   exists (");
	        sql.append(" select 1 from ACT_HI_TASKINST  t00 where "
	        		+ "	 t00.PROC_INST_ID_=t1.PROC_INST_ID_ and t00.ASSIGNEE_ in ("+userIds+") and nvl(t00.DELETE_REASON_,'') <>'deleteAdmin'");
	        sql.append(" ) ");
        }
	...
	public Integer getCount(String userId,String roleId,TaskType task) {
		try {
			...
			sqll.append(getQueryAllStr(userId,roleId,null));
