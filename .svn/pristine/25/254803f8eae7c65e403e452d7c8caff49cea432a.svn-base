...
public class TPlTaskRuleTrigAllGetter extends BaseGetter {
	...
	protected PageQueryResult getData() throws Exception {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM ( SELECT T0.* , RANK() OVER(PARTITION BY T0.accId,T0.ruleCode ORDER BY T0.trigDate DESC) RM FROM ( select T.FD_ID id,L.CUSTNAME custname,  L.LOANACNO accId  ,T.FD_RULE_ID ruleId ,T.FD_RULE_CODE ruleCode ,T.FD_RULE_NAME ruleName ,T.FD_RULE_DESC ruleDesc,T.FD_RULE_RANK ruleRank,T.FD_RULE_TYPE ruleType,T.FD_RULE_FOR ruleFor,T.FD_RULE_TRIG_TYPE ruleTrigType,T.FD_RULE_TRIG_PERIOD ruleTrigPeriod,");
        sql.append(" T.FD_RULE_TRIG_UNIT ruleTrigUnit,T.FD_TRIG_DATE trigDate ,T.FD_TRIG_TYPE trigType,T.FD_PROCESS_DATE processDate ,L.PRODNAME productName,L.TCAPI   AS loanAmount, L.OPERNAME  AS operator,L.BANKNAME AS operBank  ,L.TTERM     AS loanPeriod FROM  T_PL_TASK_RULE_TRIG T,T_EDW_PLS_ACCOUNT_V L  WHERE T.FD_ACC_ID=L.LOANACNO  AND L.PAYOFFFLAG='00' AND L.PRODID NOT IN ('93010200','96010200','96010100','93010100')  ");//显示所有 触发结果  产品名称 排出‘组合’ ，保留未结清
...
//		sql.append("  ) T0 )WHERE RM=1");
		
		//一级支行管理岗手动发起预警处置所需查询的账户
		if(!"222".equals(info.getWorkflowRoleId())){
			...
			if(StringUtils.hasText(brcodes)){
