...
function datatable1_opr_onRefresh(cell,value,record) {
	if (record) {//当存在记录时
		cell.innerHTML="<a href='JavaScript:selectIndexReport(\""+record.getValue("INDEX_CODE")+"\",\""+record.getValue("INDUSTRY_CODE")+"\",\""+record.getValue("INDUSTRY_NAME")+"\",\""+record.getValue("GATHER_TYPE")+"\",\""+record.getValue("ORG_CODE")+"\")'>查看</a>";
	} else {//当不存在记录时
		cell.innerHTML="&nbsp;";
	}
}
...
function selectIndexReport(codeVal,INDUSTRY_CODE,INDUSTRY_NAME,GATHER_TYPE,ORG_CODE){
	openSubWin("selectINDUSTRY_CODEWin"+INDUSTRY_CODE,"信贷资产监测指标展示_行业_"+INDUSTRY_NAME,"/gbicc-report-pages/combinationWarn/ftl/TCwIndexIndustryReport.ftl?INDEX_CODE="+codeVal+"&GATHER_TYPE="+GATHER_TYPE+"&INDUSTRY_CODE="+INDUSTRY_CODE+"&ORG_CODE="+ORG_CODE,"1100","750");
}
