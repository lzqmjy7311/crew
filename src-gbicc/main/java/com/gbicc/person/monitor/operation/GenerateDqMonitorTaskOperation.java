...
//				TlrInfo tlr=tlrService.getTlrInfoByTlrno(dicName);
				//20160823更改，将读取默认用户改为当前经办人,如果当前经办人也为空，则读取默认设置用户
				TlrInfo tlr=tlrService.getTlrInfoByTlrno(account.getOperid());
				if(tlr!=null){
					...
					String dicNo=FileUpDownProperties.readValue(TASK_DISTRIBUTE_MNG_CODE);
					String dicName=DataDicService.getInstance().getNameByTypeNo(dicType,dicNo);
					tlr=tlrService.getTlrInfoByTlrno(dicName);
					monitor.setHandler(tlr);
				}
	...
	public TPlDqReportJy saveJyReport(TEdwPlsAccount account,TEdwPlsCust cust,CommonService service,TPlDqMonitor monitor) throws CommonException{
		...
		//借款人抵押物信息
		String morSql="select * from t_edw_pls_mortgage where assuid in (select assuid from t_edw_pls_guarant_cust_mortgage_rel "
		+ "where loanid='"+account.getLoanid()+"' and regikind='3')";
		...
		//借款人质押物信息
		String morSql2="select * from t_edw_pls_mortgage where assuid in (select assuid from t_edw_pls_guarant_cust_mortgage_rel "
		+ "where loanid='"+account.getLoanid()+"' and regikind='4')";
