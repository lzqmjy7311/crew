...
public class TCwFinancialAllValMonitorGetter extends BaseGetter {
	...
	public PageQueryResult getData() throws Exception {
		...
		if(map.get("DATA_DATE")!=null && (String)map.get("DATA_DATE")!=""){
			Date dataDate=null;
			try{
				dataDate=sdf.parse((String)map.get("DATA_DATE"));;
			}catch(Exception e){
				dataDate=sdf2.parse((String)map.get("DATA_DATE"));;
			}
			//前台传输过来的都是月末 需增加一天
			dataDate=DateUtils.getNowDateTorDate(dataDate);
			//获取上季度最后一天加1
			nowDate=DateUtils.getNowDateLastQ(dataDate);
		}
