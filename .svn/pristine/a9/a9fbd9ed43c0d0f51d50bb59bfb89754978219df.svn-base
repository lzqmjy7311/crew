...
<script>
	...
	function btLaunchInve_onClick(button){
		...
		if(warningIds.length>0){
			...
			if(window.confirm("没有选择预警信号，是否继续？")==false){
				return false;
			}
			warningIds="0";
		}
	...
	function openInvestigationWin(casesId,warningIds,inveId){
		investigationWin=openSubWin("investigationWin","发起协查","/gbicc-com-pages/single/ftl/single_rul_investigation_add.ftl?casesId="+casesId+"&warningIds="+warningIds+"&inveId="+inveId,"1200","690");
	}
	//协查任务列表刷新触发
	...
	function btSave_onClickCheck(button){
		...
		if(casesNature==null || casesNature=="" || casesConclusion==null || casesConclusion==""){
			top.easyMsg.info("填写【案例性质】与【案例处置结论】后才可保存！");
			return false;
		}
