  ...
  //根据角色，显示或者隐藏按钮
	TaskVariable.findTaskVariable(businessId,function(y){
		...
		if(roleName=="khjlqr"){//客户经理确认
			$("div[name=group7]").hide();
			$("span[name=warnTaskEnd]").hide();
			$(btnSubmit).context.innerHTML="<span class=\"l-btn-left\"><span class=\"l-btn-text icon-default\" style=\"padding-left:20px\">确认完成</span></span>";
			$(warnTaskSubmit).context.innerHTML="<span class=\"l-btn-left\"><span class=\"l-btn-text icon-default\" style=\"padding-left:20px\">确认完成</span></span>";
	...
	if(flg){
		...
		TCmWarnDisposalAjax.isOpenNextUserWin(params,function(sy){
			...
			if(sy.success=="true"){
				if(sy.showWin=="true"){
					TaskVariable.findTaskVariable(businessId,function(y){
						if(y && y.nextRole && roleName!="fhfxjczg"){//获取到角色ID打开窗口。
					...
					if(isSubmitNext=='1'&&roleName=="fhfxjczg"){
						submitWindow=openSubWin("submitWindow","任务发送至","/gbicc-pages/bpm/ftl/task_assignee.ftl?orgId=00001&roleId=560","600","400");
					}else{
						warnTaskSubmit.click();
					}
				}
...
function addWarnBtn_onClick(button) {
	var businessId='${taskId}';
	selectWarnWin=openSubWin("selectWarnWin","人工发起预警》定性规则","/gbicc-com-pages/cmWarnDisposal/ftl/TCmRulDefinitionWin.ftl?businessId="+businessId,"900","700");
}
